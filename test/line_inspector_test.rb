...
class LineInspectorTest < Sherpa::Test
  ...
  # Currently only supports vim, not sure about other markers
  test "Trims fold markers generated by an editor" do
    slash_open = "//~// {{{"
    slash_close = "some text// }}}"
    slash_open_alt = "//~//{{{"
    slash_close_alt = "some text//}}}"
    pound_open = "#~# {{{"
    pound_close = "some text# }}}"
    multi_open = "/*~//{{{"
    multi_close = "*/// }}}"
    java_open = "/**~//{{{"
    slash_first = "//~ First:// {{{"
    slash_first_alt = "//~ First://{{{"
    pound_first = "#~ First:# {{{"
    multi_first = "/*~ First://{{{"
    java_first = "/**~ First://{{{"

    assert_equal @inspector.trim_fold_markers(slash_open), "//~"
    assert_equal @inspector.trim_fold_markers(slash_close), "some text"
    assert_equal @inspector.trim_fold_markers(slash_open_alt), "//~"
    assert_equal @inspector.trim_fold_markers(slash_close_alt), "some text"
    assert_equal @inspector.trim_fold_markers(pound_open), "#~"
    assert_equal @inspector.trim_fold_markers(pound_close), "some text"
    assert_equal @inspector.trim_fold_markers(multi_open), "/*~"
    assert_equal @inspector.trim_fold_markers(multi_close), "*/"
    assert_equal @inspector.trim_fold_markers(java_open), "/**~"
    assert_equal @inspector.trim_fold_markers(slash_first), "//~ First:"
    assert_equal @inspector.trim_fold_markers(slash_first_alt), "//~ First:"
    assert_equal @inspector.trim_fold_markers(pound_first), "#~ First:"
    assert_equal @inspector.trim_fold_markers(multi_first), "/*~ First:"
    assert_equal @inspector.trim_fold_markers(java_first), "/**~ First:"
  end
